...
v1 = c(1,2,NA,NA,5)
is.na(v1)           #missing values founder
mean(v1, na.rm=T)
v1a = na.omit(v1)
sum(v1a)
?na.omit
...
sleep[complete.cases(sleep),]   #Computes in dataset
sleep[!complete.cases(sleep),]
sum(is.na(sleep$Dream))         #computes in vectors
mean(is.na(sleep$Dream))
12/62
mean(!complete.cases(sleep))    #percent of complete cases
sum(is.na(sleep))
colSums(is.na(sleep))
rowSums(is.na(sleep))
